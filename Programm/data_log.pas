...
s:='SELECT `date_insert`,`sensor_id`,`rashod` FROM `records` where `date_insert` BETWEEN '+QuotedStr(s1)+' and '+QuotedStr(s2)+';';
 ...
 for i := StringGrid1.FixedRows to StringGrid1.RowCount - 1 do StringGrid1.Rows[i].Clear;
    ...
    Form1.SQLQuery2.SQL.Add  ('SELECT `date_insert`,`sensor_id`,`rashod` FROM `records` where `date_insert` BETWEEN '+QuotedStr(s1)+' and '+QuotedStr(s2)+';') ;
